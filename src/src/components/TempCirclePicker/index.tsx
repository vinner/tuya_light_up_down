...
export default class TempCirclePicker extends React.Component<TempCirclePickerProps, any> {
  ...
  handleSetPanResponder = (e: GestureResponderEvent) => {
    ...
    if (len >= innerRadius && len <= outerRadius) {
      ...
      if (this.props.moveDisable && this.props.onRelease) {
  ...
  handleValue(value: number) {
    ...
    this.thumbX = x;
    this.thumbY = y;
    //TYSdk.native.simpleTipDialog('temp: ' + this.thumbX + ',' + this.thumbY + ',' + value, () => {});
  }
  ...
  render() {
    ...
    //TYSdk.native.simpleTipDialog('temp: ' + thumbX + ',' + thumbY + ',' + this.props.value, () => {});
    return (
