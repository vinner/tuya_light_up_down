...
export default class Slider extends Component {
  ...
  _testValue(value, props) {
    ...
    return Utils.NumberUtils.inMaxMin(props.minimumValue, props.maximumValue, v);
  }
  ...
  renderMaxNounView = () => {
    ...
    const time = Math.floor((maximumValue - minimumValue) / nounStep);
    if (horizontal) {
      if (type === 'parcel') {
        return _.times(time, n => {
          return (
            <View
              key={n}
              style={[
  ...
  renderMinNounView = () => {
    ...
    const actualNounNum = reverseValue
      ? Math.floor(maximumValue - actualValue) / nounStep + 1
      : Math.floor(actualValue - minimumValue) / nounStep + 1;
    const time = Math.floor(maximumValue - minimumValue) / nounStep;
    if (horizontal) {
      if (type === 'parcel') {
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {!!nounStep && !!useNoun && this.renderMaxNounView()}
        {!onlyMaximumTrack && (
          <Animated.View
            style={[
              {
        ...
        {!!nounStep && !!useNoun && reverseValue && type !== 'parcel' && this.renderMaxNounView()}
        {!!nounStep && !!useNoun && this.renderMinNounView()}
        <Animated.View
