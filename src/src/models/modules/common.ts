...
const updateUi = createAction('CHANGE_UI');
...
const cloudState = handleActions(
  {
    [initCloudState.toString()]: (state, action) => {
      ...
      const sceneBright = payload.sceneBright || state.sceneBright;
      const singleTimer = payload.singleTimer || state.singleTimer;
      const singleColors = payload.singleColors || state.singleColors;
      const groupColors = payload.groupColors || state.groupColors;
      const lightColors = payload.lightColors || state.lightColors;
      const diyScenes = payload.diyScenes || state.diyScenes;
      const lightUpDownSwitch = payload.lightUpDownSwitch || state.lightUpDownSwitch;

      return {
        ...state,
        ...payload,
        sceneDatas: [...customScenes],
        ...
        diyScenes: diyScenes,
        lightUpDownSwitch: lightUpDownSwitch,
      };
    },
