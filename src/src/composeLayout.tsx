...
const composeLayout = (store: Store, component: React.ComponentType) => {
  ...
  class PanelComponent extends Component<Props> {
    ...
    componentWillMount() {
      ...
      let defaultStaticScenes = defaultCustomScenes;
      ...
      getDeviceCloudData()
        .then((cloudData: any) => {
          ...
          //TYSdk.native.simpleTipDialog('cloud: ' + JSON.stringify(cloudData), () => {});

          customScenes = parseJSON(customScenes);
          sceneBright = parseInt(sceneBright);
          singleTimer = parseJSON(singleTimer);
          groupColors = parseJSON(groupColors);
          diyScenes = parseJSON(diyScenes);
          lightUpDownSwitch = parseInt(lightUpDownSwitch);
          const isEmptyObj = obj => Object.keys(obj).length === 0;

          const newSceneStates = {
            customScenes: isEmptyObj(customScenes) ? defaultCustomScenes : [...customScenes],
